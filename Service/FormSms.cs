...
namespace Service
{
    public partial class FormSms : Form
    {
        ...
        public string Add_PhoneNumber_To_Text(DataGridView dg)
        {
            txtMobilesNumbers.Text = "";
            ...
            for (int i = 0; i < dg.RowCount; i++)
            {
                ...
                mobile = dg.Rows[i].Cells[3].Value.ToString();
        ...
        private void btnSearchBirthday_Click(object sender, EventArgs e)
        {
            try
            {
                using (var context = new StimulsoftEntities())
                {
                    ...
                    findDay = findDay.Substring(4, 4);
                    date = int.Parse(findDay);
                    var tavalod = context.User.Where(c => c.BirthDayDate.ToString().Substring(4, 4) == findDay).ToList();
                    dgSearch.DataSource = tavalod;
                    lblUserCount.Text = dgSearch.RowCount.ToString();
                }
                Design_GrideView_User();
                txtMobilesNumbers.Text = Add_PhoneNumber_To_Text(dgSearch); // ezafe kardane shomare
        ...
        private void dgSearch_CellClick(object sender, DataGridViewCellEventArgs e)
        {
            ...
            //if (x == 0)
            //{
            //    var AllUser = context.User.OrderByDescending(c => c.Eshterak).ToList();
            //    dgSearch.DataSource = AllUser;
            //    x = 1;
            //}
            //else
            //{
            //    var AllUser = context.User.OrderBy(c => c.Eshterak).ToList();
            //    dgSearch.DataSource = AllUser;
            //    x = 0;
            //}
